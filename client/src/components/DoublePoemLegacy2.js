...
const ResponsiveText = (props) => {
    ...
    const textAlignOffset = leftWidth / 2.5 //385 //should probably measure this. it's the inner left-view size, because we flipped textAlign the font hovers deceptively on the other side of the div
    ...
    const t = true
    const performanceMode = perfMode != null && perfMode.length > 0 ? true : false
    const { fontSizeNormal, fontSizeHover, fontSizeCenter, fontSizeUnderline, fFamilyCenter, fFamilyNormal, fWeightCenter, fWeightNormal, dropShadowPX,fColorLeft, fColorRight, fColorTouching, fBlurColor, fMinWidth, fCenterOffset, backgroundColor, heart1Color, heart2Color, noiseContrast, noiseBrightness, noisePosLeft, noiseFreq, noiseOctaveNum, viewboxW, viewboxH, bFollowCursorNoise, dropShadowColor, noiseRotate } = 
            useControls({ 'Text/Font': folder({fontSizeNormal: 15, fontSizeHover: 20, fontSizeCenter: 20, fontSizeUnderline: 14, fFamilyCenter: 'Syne', fFamilyNormal: 'Kanit', fWeightCenter: 500, fWeightNormal: 100, dropShadowPX: 6.5, dropShadowColor: '#f2baba',
    ...
    const noiseStyles2 = useSpring({
        ...
        from: { position: 'absolute', filter: `contrast(${noiseContrast}%) brightness(${noiseBrightness}%)`, background: `radial-gradient(circle at ${noiseLogic}% ${isCenter ? 50 : 25}%, rgba(0,0,255,1), rgba(216,39,39,0))`, rotateZ: 0, transform: 'scale(1.3)', marginTop: '0px' },
        to: async next => {
          while (1) {
            ...
            await next({ position: 'absolute', filter: `contrast(${noiseContrast}%) brightness(${40}%)`, background: `radial-gradient(circle at ${noiseLogic}% ${40}%, rgba(0,0,255,1), rgba(216,39,39,0))`, rotateZ: 0, transform: 'scale(1.1)', marginTop: '0px' })
            await next({ position: 'absolute', filter: `contrast(${noiseContrast}%) brightness(${55}%)`, background: `radial-gradient(circle at ${noiseLogic}% ${25}%, rgba(0,0,255,1), rgba(216,39,39,0))`, rotateZ: noiseRotate ? 180 - 45: 0, transform: 'scale(0.40)', marginTop: '-40px' })
            await next({ position: 'absolute', filter: `contrast(${noiseContrast}%) brightness(${75}%)`, background: `radial-gradient(circle at ${noiseLogic}% ${10}%, rgba(0,0,255,1), rgba(216,39,39,0))`, rotateZ: 0, transform: 'scale(1.2)', marginTop: '0px' })
          }
        },
      })
    ...
    const marginRightLogic = isCenter ? (leftWidth * 0.5) :
        (overRightPane ? 
            (mousePos.x >= leftWidth + leftWidth * 0.84 ? leftWidth * 0.33 : 
                (mousePos.x <= leftWidth + leftWidth * 0.125 ? leftWidth * 0.6 : leftWidth + leftWidth - mousePos.x - rightTextAlignOffset)) 
    ...
    const animationStylesRight = useSpring({
        ...
        marginRight: 
            isCenter ? `${marginRightLogic - (leftWidth * 0.025)}px` : `${marginRightLogic}px`,
    ...
    return (
      ...
      <a.div className={"dblPoemBackground"} style={BackgroundStyles} >
        ...
        <a.svg viewBox={"0 0 60 60"} class='svgHeart' style={svgHeartStyles} xmlns="http://www.w3.org/2000/svg">
        <defs>
        ...
        <g>
          <path clip-path="url(#the-object)" fill="red" d="M23.6,0c-3.4,0-6.3,2.7-7.6,5.6C14.7,2.7,11.8,0,8.4,0C3.8,0,0,3.8,0,8.4c0,9.4,9.5,11.9,16,21.2
            c6.1-9.3,16-12.1,16-21.2C32,3.8,28.2,0,23.6,0z" width="100%" height="100%" filter="url(#noiseFilter)" 
            />
        </g>
        </a.svg>

        <a.svg viewBox={overLeftPane ? "0 0 60 60" : "0 0 60 60"} class='svgHeart2' style={svgHeartStyles2} xmlns="http://www.w3.org/2000/svg">
        <defs>
        ...
        <g>
          <path clip-path="url(#the-object2)" fill="red" d="M23.6,0c-3.4,0-6.3,2.7-7.6,5.6C14.7,2.7,11.8,0,8.4,0C3.8,0,0,3.8,0,8.4c0,9.4,9.5,11.9,16,21.2
            c6.1-9.3,16-12.1,16-21.2C32,3.8,28.2,0,23.6,0z" width="100%" height="100%" filter="url(#noiseFilter3)" 
            />
        </g>
        </a.svg>

        <a.svg viewBox={viewBox} style={isCenter ? noiseStyles2 : noiseStyles} class='noisebg2' xmlns="http://www.w3.org/2000/svg">
            <filter id="noiseFilter2">
            <feTurbulence
              type="fractalNoise"
              baseFrequency={noiseFreq}
              numOctaves={noiseOctaveNum}
              stitchTiles="stitch"
            />
          </filter>
